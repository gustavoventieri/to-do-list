...
import br.com.gustavomendes.todolist.utils.Utils;
...
public class TaskController {
    ...
    public ResponseEntity update(@RequestBody TaskModel taskModel, HttpServletRequest request, @PathVariable UUID id) {     
        ...
        if (task == null) {
              return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Tarefa nâo encontrada");
        }
        if (!task.getIdUser().equals(idUser)) {
              return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("O Usuario não tem permissão para alterar esta tarefa");
        }
